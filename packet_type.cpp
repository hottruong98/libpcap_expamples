...
void my_packet_handler(u_char* args, const struct pcap_pkthdr* header, const u_char* packet) {
    struct ether_header* eth_header; // Ethernet header or MAC header (14 bytes)
    eth_header = (struct ether_header*) packet; // treat pointer to packet as pointer to Ehternet header
